...
async function showAllDemandsAndMissions() {
  ...
  try {
    ...
    allDemands.rows.forEach(demand => {
      ...
      console.log(`${statusColor} ID: ${demand.id} | ${demand.expediteur_name} | Agency: ${demand.expediteur_agency} | Status: ${demand.status} | Created: ${demand.created_at}`);
    });
    ...
    const allMissions = await client.query(`
      SELECT id, driver_name, status, created_at, completion_code
    ...
    allMissions.rows.forEach(mission => {
      ...
      console.log(`${statusColor} Mission #${mission.id} | Driver: ${mission.driver_name} | Status: ${mission.status} | Code: ${mission.completion_code || 'N/A'} | Created: ${mission.created_at}`);
    });
    ...
    const missionDemands = await client.query(`
      SELECT md.mission_id, md.demand_id, pm.status as mission_status, pm.driver_name,
    ...
    const assignedDemands = await client.query(`
      SELECT DISTINCT d.id, d.expediteur_name, d.expediteur_agency, d.status,
             md.mission_id, pm.status as mission_status, pm.driver_name
      FROM demands d
    ...
    assignedDemands.rows.forEach(demand => {
      console.log(`🔗 Demand #${demand.id} (${demand.expediteur_name}) -> Mission #${demand.mission_id} (${demand.mission_status}) - Driver: ${demand.driver_name}`);
    });
    ...
    const completedMissions = await client.query(`
      SELECT id, driver_name, status, completion_code, created_at
      FROM pickup_missions 
    ...
    completedMissions.rows.forEach(mission => {
      console.log(`🎉 Mission #${mission.id} | Driver: ${mission.driver_name} | Status: ${mission.status} | Code: ${mission.completion_code || 'N/A'}`);
    });
